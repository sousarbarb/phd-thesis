...
A crescente adoção de robôs móveis autónomos~(AMRs) nas indústrias de
logística, intralogística, retalho e manufatura é uma motivação para melhorar a robustez dos
...
da pose do robô quando estas observações dinâmicas são integradas sem
tratamento adequado. Esta Tese de Doutoramento, intitulada
...
exclusivamente a dados de um \emph{scanner} laser 2D e, opcionalmente, a
odometria das rodas como modalidades de perceção.
...
experimentais evidenciam que as características extraídas são invariantes ao
ponto de vista e à geometria do ambiente. Segundo, introduz-se uma
...
até $O(1)$ numa representação de mapa de distâncias pré-calculada.
...
Por fim, desenvolve-se a \emph{framework} RicoSLAM propriamente dita, que
...
A \emph{framework} inclui critérios de divisão (\emph{split}) e de fusão
(\emph{merge}) baseados na razão de \emph{inliers} para a gestão de
\emph{keyframes}, um mecanismo de deteção de visitas a lugares já mapeados que
combina proximidade no referencial global com
...
ocupação~2D. Adicionalmente, propõem-se fatores de Global Bundle
Adjustment~(GBA) baseados em mapas de distância, que
...
ponto-a-plano para alinhamento de nuvens de pontos 2D.
...
consideradas, mantendo-se o modo de mapas locais abaixo de
\SI{0.05}{\metre} em todas elas, com a correção de pose \emph{offline} a
reduzir ainda mais o erro nas sequências com múltiplas revisitas ao mesmo
local.
...
dinâmica é avaliado em sequências adicionais de \emph{datasets} de AMRs em
ambientes industriais e académicos~(Avantgarde, Cappero, Flowbotic e Kuka),
...
Por fim, um \emph{benchmark} sintético valida a formulação dos fatores de GBA
propostos como uma etapa de otimização global do grafo de poses do
\emph{back-end}, ficando a sua integração com a operação \emph{online} do
RicoSLAM e a respetiva validação em ambientes reais como trabalho futuro.
...
The growing deployment of Autonomous Mobile Robots~(AMRs) in logistics,
intralogistics, retail,
...
returns are integrated without proper handling. This PhD~Thesis,
entitled
\textit{RicoSLAM: localization and mapping in dynamic environments},
investigates how environment dynamics can be incorporated into both
localization and mapping in a 2D~SLAM framework using 2D~laser scanner data
and, optionally, wheeled odometry as the sensing modalities.
...
maintains the local map of each keyframe by discarding the map points that a
later beam sees through and by adding the newly observed returns, and an
offline ray-based stage to suppress dynamic
...
planar sequences considered, with its local map operating mode staying below
\SI{0.05}{\metre} on all of them, and with an offline pose-correction stream
that further reduces the error on sequences with multiple revisits to the same
location. The
...
The offline dynamics removal mechanism is additionally evaluated on sequences
from industrial and academic AMR deployments~(Avantgarde, Cappero, Flowbotic,
and Kuka), consistently sharpening the empirical occupancy
distribution of the resulting 2D~grid map and reclassifying as
\emph{unknown} the cells whose only supporting observations were transient
dynamic returns. Lastly, a synthetic Manhattan-like benchmark validates the
formulation of the proposed distance map-based GBA factors as a globally
consistent refinement stage for the back-end pose graph, leaving their
integration with the online operation of RicoSLAM and the corresponding
real-world validation as future work.

...
let $\mathbf{x}$ denote non-observable state variables and
$\mathbf{z}$ as the measurements.
...
that is, the posterior $p(\mathbf{x}|\mathbf{z})$~\cite{grisetti:robotics:2020}.
...
By applying Bayes' rule, as shown in
\eqref{eq:distance-maps:background:least-squares:bayes}, the posterior
$p(\mathbf{x}|\mathbf{z})$ becomes proportional to
the likelihood distribution of the \emph{observation model} given the state
$p(\mathbf{z}|\mathbf{x})$ multiplied by the prior $p(\mathbf{x})$.
Assuming an uniform prior distribution over the state (meaning that
all admissible values of $\mathbf{x}$ are considered equally likely), then, the
posterior $p(\mathbf{x}|\mathbf{z})$ depends only on the measurement
distribution conditioned on the state $p(\mathbf{z}|\mathbf{x})$.
...
p(\mathbf{z}^{[i]}|\mathbf{x})
  & = \mathcal{N}(\mathbf{z}^{[i]} ; \mathbf{h}^{[i]}(\mathbf{x}), \Sigma^{[i]})\\
  & \propto \exp\left(-\left( \mathbf{h}^{[i]}(\mathbf{x}) - \mathbf{z}^{[i]} \right)^T
                      \left(\Sigma^{[i]}\right)^{-1}
                      \left( \mathbf{h}^{[i]}(\mathbf{x}) - \mathbf{z}^{[i]} \right)
                \right)\\
  & \propto \exp\left( -\mathbf{e}^{[i]}(\mathbf{x})^T \Omega^{[i]} \mathbf{e}^{[i]}(\mathbf{x}) \right)
\end{aligned}
\end{equation}
...
\mathbf{x}^*
  ...
  & = \argmax_\mathbf{x} \prod_i \exp\left( -\mathbf{e}^{[i]}(\mathbf{x})^T \Omega^{[i]} \mathbf{e}^{[i]}(\mathbf{x}) \right)\\
  & = \argmin_\mathbf{x} \sum_i \mathbf{e}^{[i]}(\mathbf{x})^T \Omega^{[i]} \mathbf{e}^{[i]}(\mathbf{x})
\end{aligned}
\end{equation}
